...
class _PropriedadesScreenState extends State<PropriedadesScreen> {
  ...
  Future<void> _refreshData() async {
    ...
    try {
      final propriedades =
          await SQLiteHelper.instance.readTopLevelPropriedades();
      ...
      if (mounted) {
        setState(() {
          ...
          _allEguas = eguas;
          _propriedadesComManejosAgendados =
              manejosAgendados.map((m) => m.propriedadeId).toSet();
          _filterData();
        });
      }
    } catch (e) {
                              ...
                              child: ListTile(
                                ...
                                onTap: () {
                                  Navigator.push(
                                    ...
                                    MaterialPageRoute(
                                      builder: (context) =>
                                          SubPropriedadesListScreen(
                                        parentPropriedade: prop,
                                      ),
                                    ),
                                  ).then((_) {
                                    _refreshData();
                          ...
                          onPressed: () async {
                            if (formKey.currentState!.validate()) {
                              final novaPropriedade = Propriedade(
                                ...
                                parentId: null, // Garante que é uma propriedade-mãe
                              );
